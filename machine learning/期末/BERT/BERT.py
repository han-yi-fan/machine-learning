...
# 读取预训练的模型参数
modelConfig = BertConfig.from_pretrained('F:\\机器学习大作业\\uncased_L-12_H-768_A-12\\bert_config.json')
model = BertForSequenceClassification.from_pretrained('F:\\机器学习大作业\\uncased_L-12_H-768_A-12', config=modelConfig)
...
for batch in test_dataloader:
    ...
    nb_eval_steps += 1
print("Validation Accuracy: {}".format(test_accuracy / nb_eval_steps))
...
for batch in test_dataloader:
    ...
    # 模型结果矩阵
    logits = logits.detach().cpu().numpy()
np.savetxt('测试结果.txt', logits, fmt="%d", delimiter=" ")
